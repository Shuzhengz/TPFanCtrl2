...
class CTaskbarTextIcon
{
	private:
		...
		void _createdicon(const int iFarbeIconC, LPCTSTR aTooltipC) 
			{	
			if(m_pSystray) m_pSystray->SetTooltipText(aTooltipC);
			CDynamicIcon* pDiconTemp = new CDynamicIcon(m_line1, m_line2, iFarbeIconC);
    ...
    CTaskbarTextIcon
        ...
        m_BgTransparent(false)        
			...
			_createdicon(iFarbeIconC,aTooltipC);
