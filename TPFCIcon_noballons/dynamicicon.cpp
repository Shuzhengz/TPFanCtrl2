...
CDynamicIcon::CDynamicIcon(const char *line1,const char *line2, const int iFarbeIconA) {
    //3 chars per line
  ...
  break;
  case 15:
  hBrush = CreateSolidBrush (RGB (0,0,0)); //schwarz
  break;
  case 21:
...
//#define DEV_FIND_FONT
   ...
   SecureZeroMemory(&lf,sizeof(LOGFONT));
    ...
    lf.lfHeight = -MulDiv(6, GetDeviceCaps(hDC, LOGPIXELSY), 64); // 64 neu
    lf.lfWeight=400;
    lf.lfOutPrecision = 1;
